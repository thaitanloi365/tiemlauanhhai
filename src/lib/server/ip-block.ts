...
function isRecordActive(record: Pick<BlockedIpRecord, 'expires_at'>, now = Date.now()) {
	if (!record.expires_at) return true;
	return new Date(record.expires_at).getTime() > now;
}

function setCachedStatus(ip: string, blocked: boolean, reason: string | null, expiresAt: string | null) {
	blockStatusCache.set(ip, { blocked, reason, expiresAt, cachedAt: Date.now() });
}
...
function cleanupLocalState() {
	...
	for (const [ip, timestamps] of violationsByIp.entries()) {
		const recent = timestamps.filter((timestamp) => timestamp > now - AUTO_BLOCK_WINDOW_MS);
		if (recent.length === 0) {
...
async function getSupabaseBlockedIp(ip: string): Promise<BlockedIpRecord | null> {
...
export async function isIpBlocked(ipInput: string) {
	...
	if (!ip) return { blocked: false, reason: null as string | null, expiresAt: null as string | null };

	const cached = getCachedStatus(ip);
	if (cached) {
		return { blocked: cached.blocked, reason: cached.reason, expiresAt: cached.expiresAt };
	}

	const localRecord = localBlockedIps.get(ip);
	if (localRecord && isRecordActive(localRecord)) {
		setCachedStatus(ip, true, localRecord.reason ?? 'Blocked', localRecord.expires_at);
		return { blocked: true, reason: localRecord.reason, expiresAt: localRecord.expires_at };
	}
	...
	return { blocked: true, reason: supabaseRecord.reason, expiresAt: supabaseRecord.expires_at };
}
...
export async function listBlockedIps() {
	...
	if (!hasSupabaseConfig()) {
		return Array.from(localBlockedIps.values()).sort((a, b) => (a.created_at < b.created_at ? 1 : -1));
	}
...
export async function unblockIpById(id: string) {
	const localEntry = Array.from(localBlockedIps.entries()).find(([, value]) => value.id === id);
	if (localEntry) {
		const [ip] = localEntry;
		localBlockedIps.delete(ip);
