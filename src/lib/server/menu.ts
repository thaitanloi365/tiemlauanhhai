...
export async function getMenuItemBySlug(slug: string) {
  ...
  const categoryById = new Map(
    menu.categories.map((category) => [category.id, category.slug]),
  );
  const item = menu.menuItems.find((entry) => entry.slug === slug);
  const relatedItems = menu.menuItems
    .filter(
      (entry) =>
        entry.category_id === item?.category_id && entry.id !== item?.id,
    )
    .slice(0, 4);
  const toppings = menu.menuItems.filter(
    (entry) =>
      categoryById.get(entry.category_id) === 'topping' &&
      entry.variants.length > 0,
  );
  const drinks = menu.menuItems.filter(
    (entry) =>
      categoryById.get(entry.category_id) === 'do-uong' &&
      entry.variants.length > 0,
  );
