...
const mockOrderLogs: { id: string; order_id: string; status: string; created_at: string }[] = [];
...
export const mockDb = {
	...
	getOrderDetail(orderId: string, sessionId?: string) {
		...
		const items = mockOrderItems
			...
			.map((entry) => {
				const variant = mockMenuItems.flatMap((item) => item.variants).find((item) => item.id === entry.menu_variant_id);
				const menuItem = mockMenuItems.find((item) => item.id === variant?.menu_item_id);
				return {
					...entry,
	...
	updateOrder(orderId: string, updates: Partial<Order>) {
		...
		mockOrders[idx] = { ...mockOrders[idx], ...updates, updated_at: new Date().toISOString() };
		if (updates.status) {
			mockOrderLogs.push({
				id: crypto.randomUUID(),
				order_id: orderId,
	...
	createReview(review: Review) {
		const existed = mockReviews.find((entry) => entry.order_id === review.order_id);
