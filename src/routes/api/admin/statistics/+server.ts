...
type OrderStatsRow = Pick<Order, 'id' | 'total_amount' | 'created_at' | 'status' | 'district' | 'ward'>;
...
function aggregateStatistics(
	...
	const monthlyMap = new Map<number, { month: number; total: number; count: number }>();
	for (let month = 1; month <= 12; month += 1) {
		monthlyMap.set(month, { month, total: 0, count: 0 });
	}

	const districtMap = new Map<string, { district: string; count: number; total: number }>();
	const wardMap = new Map<string, { district: string; ward: string; count: number; total: number }>();
	...
	const monthly = filters.month
		? [monthlyMap.get(filters.month) ?? { month: filters.month, total: 0, count: 0 }]
	...
	return {
		...
		ordersByArea: {
			byDistrict: Array.from(districtMap.values()).sort((a, b) => b.count - a.count || b.total - a.total),
			byWard: Array.from(wardMap.values()).sort((a, b) => b.count - a.count || b.total - a.total)
		},
		summary: {
			totalOrders,
			totalRevenue,
...
export async function GET({ url, locals }: RequestEvent) {
	...
	if (!hasSupabaseConfig()) {
		...
		return json(aggregateStatistics(rows, filters));
	}
	...
	return json(aggregateStatistics((data ?? []) as OrderStatsRow[], filters));
}
