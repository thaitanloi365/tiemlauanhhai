...
const RULES = {
	...
	reviewsPost: { limit: 5, windowMs: 15 * 60 * 1000, keyPrefix: 'reviews:post' },
	adminLoginPost: { limit: 5, windowMs: 15 * 60 * 1000, keyPrefix: 'admin-login:post' },
...
function resolveRule(pathname: string, method: string): RateLimitRule | null {
	...
	if (method === 'POST' && pathname === '/api/reviews') return RULES.reviewsPost;
	if (method === 'POST' && pathname === '/api/admin/auth/login') return RULES.adminLoginPost;
...
export function consumeIpRateLimit(input: {
	...
	const recentHits = (hitsByKey.get(storageKey) ?? []).filter((timestamp) => timestamp > windowStart);
	recentHits.push(now);
	...
	const retryAfterSeconds = ok ? 0 : Math.max(1, Math.ceil((resetAt - now) / 1000));
